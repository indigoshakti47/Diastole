...
const MapContainer = ({ google, citizens }) => {
 ...
 const animationPlane = (map, target) => {
  ...
  return (
    <Map google={google} zoom={14} onReady={_onMapLoaded} >
      {
        citizens.map(c => <Marker position={{ lat: c.location.lat, lng: c.location.lng }} key={c.document_number} onClick={onMarkerClick}
